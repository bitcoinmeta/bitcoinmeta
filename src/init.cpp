 ...
 */
static const char* BITCOIN_PID_FILENAME = "bitcoinmetad.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("bitcoinmeta-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinmeta-project/bitcoinmeta>";
    const std::string URL_WEBSITE = "<https://bitcoinmeta.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitcoinmeta-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitcoinmeta is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoinmeta is started while in a temporary directory.\n",
