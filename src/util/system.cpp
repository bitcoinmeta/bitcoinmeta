...
const char * const BITCOIN_CONF_FILENAME = "bitcoinmeta.conf";
...
#else
    const char* pszModule = "bitcoinmeta";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinMeta";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinMeta";
#else
    // Unix
    return pathRet / ".bitcoinmeta";
#endif
#endif
}
